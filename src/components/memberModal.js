...
const MemberModal = ({ isVisible, onClose, onAdd, onUpdate, isUpdate, memberToEdit }) => {
  ...
  const resetModalState = () => {
    if (isUpdate) {
      ...
      setSelectedDate(new Date(memberToEdit.dob));
    } else {
  ...
  const handleDateChange = (event, selectedDate) => {
    ...
    if (selectedDate) {
      ...
      setNewMember({ ...newMember, dob: selectedDate.toISOString() });
    }
  };
      ...
      <View style={styles.modalContainer}>
        <View style={styles.modalContent}>
          ...
          {isUpdate ? (
            ...
            <Text style={styles.input}>{newMember.dob ? new Date(newMember.dob).toLocaleDateString() : ''}</Text>
            </TouchableOpacity>
          ) : (
            <TouchableOpacity onPress={showDatePickerModal}>
